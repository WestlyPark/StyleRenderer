...
if __name__ == "__main__":
	...
	parser.add_argument('--size', type = int, default = 256, \
		help =	'image sizes used for embedding calculation [%(default)d]')
	parser.add_argument('--batch', type = int, default = 64, \
		help =	'batch size for inception networks [%(default)d]')
	parser.add_argument('--n_sample',type = int, default = 50000, \
		help =	'number of samples used for embedding calculation [%(default)d]')
	parser.add_argument('--flip', action = 'store_true', \
		help =	'apply random flipping to real images')
	...
	inception = nn.DataParallel(inception).eval().to(args.device)
	...
	features = extract_features(loader, inception, args.device).numpy()
	features = features[: args.n_sample]
	print('extracted %d features' % features.shape[0])
	mean = np.mean(features, 0)
	cov = np.cov(features, rowvar = False)
	name = os.path.splitext(os.path.basename(args.path))[0]
	data = {'mean': mean, 'cov': cov, 'size': args.size, 'path': args.path}
	try:
		import pickle
		with open('inception_%s.pkl' % name, 'wb') as f:
			pickle.dump(data, f)
	except:
		with open('inception_%s.txt' % name, 'wb') as f:
			f.write('mean:\t%f\n' % mean)
			f.write('cov:\t%f\n' % cov)
			f.write('size:\t%f\n' % args.size)
			f.write('path:\t%s' % args.path)
